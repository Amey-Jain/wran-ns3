...
NS_LOG_COMPONENT_DEFINE ("WranMulticastSimulation");
...
{/*
  ...
  Ptr<SimpleOfdmWranChannel> channel;
  ...
  channel = CreateObject<SimpleOfdmWranChannel> ();
  channel->SetPropagationModel (SimpleOfdmWranChannel::COST231_PROPAGATION);
  ssDevs = wran.Install (ssNodes,
                          WranHelper::DEVICE_TYPE_SUBSCRIBER_STATION,
                          WranHelper::SIMPLE_PHY_TYPE_OFDM,
                          channel,
                          scheduler);
  Ptr<WranNetDevice> dev = wran.Install (bsNodes.Get (0),
  ...
  ServiceFlow MulticastServiceFlow = wran.CreateServiceFlow (ServiceFlow::SF_DIRECTION_DOWN,
                                                              ServiceFlow::SF_TYPE_UGS,
                                                              MulticastClassifier);

  bs->GetServiceFlowManager ()->AddMulticastServiceFlow (MulticastServiceFlow, WranPhy::MODULATION_TYPE_QPSK_12);
