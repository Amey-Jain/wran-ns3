 ...
 */
class WranUplinkSchedulerMBQoS : public WranUplinkScheduler
{
public:
  WranUplinkSchedulerMBQoS ();
  WranUplinkSchedulerMBQoS (Time time);
  ~WranUplinkSchedulerMBQoS (void);
   ...
   */
  void EnqueueJob (WranUlJob::JobPriority priority, Ptr<WranUlJob> job);
   ...
   */
  Ptr<WranUlJob> DequeueJob (WranUlJob::JobPriority priority);
   ...
   */
  Time DetermineDeadline (WranServiceFlow *serviceFlow);
   ...
   */
  uint32_t CountSymbolsQueue (std::list<Ptr<WranUlJob> > jobs);
   ...
   */
  uint32_t CountSymbolsJobs (Ptr<WranUlJob> job);
   ...
   */
  Ptr<WranUlJob>
  CreateWranUlJob (WranSSRecord *ssRecord, enum WranServiceFlow::SchedulingType schedType, ReqType reqType);

  uint32_t
  GetPendingSize (WranServiceFlow* serviceFlow);

  bool
  ServiceBandwidthRequestsBytes (WranServiceFlow *serviceFlow,
...
private:
  ...
  std::list<Ptr<WranUlJob> > m_uplinkJobs_high;
  std::list<Ptr<WranUlJob> > m_uplinkJobs_inter;
  std::list<Ptr<WranUlJob> > m_uplinkJobs_low;
