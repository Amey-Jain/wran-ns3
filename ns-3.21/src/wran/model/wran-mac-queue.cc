...
NS_LOG_COMPONENT_DEFINE ("WranMacQueue");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (WranMacQueue);

WranMacQueue::QueueElement::QueueElement (void)
...
WranMacQueue::QueueElement::QueueElement (Ptr<Packet> packet,
...
uint32_t
WranMacQueue::QueueElement::GetSize (void) const
{
...
TypeId
WranMacQueue::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WranMacQueue")
    .SetParent<Object> ()
    .AddAttribute (
      "MaxSize",
      "Maximum size",
      UintegerValue (1024),
      MakeUintegerAccessor (&WranMacQueue::GetMaxSize,
                            &WranMacQueue::SetMaxSize),
      MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Enqueue",
                     "Enqueue trace",
                     MakeTraceSourceAccessor (&WranMacQueue::m_traceEnqueue))
    .AddTraceSource ("Dequeue",
                     "Dequeue trace",
                     MakeTraceSourceAccessor (&WranMacQueue::m_traceDequeue))
    .AddTraceSource ("Drop",
                     "Drop trace",
                     MakeTraceSourceAccessor (&WranMacQueue::m_traceDrop))
  ;
  return tid;
}

WranMacQueue::WranMacQueue (void)
  : m_maxSize (0),
    m_bytes (0),
    m_nrDataPackets (0),
...
WranMacQueue::WranMacQueue (uint32_t maxSize)
  : m_maxSize (maxSize),
    m_bytes (0),
    m_nrDataPackets (0),
    m_nrRequestPackets (0)
...
void
WranMacQueue::SetMaxSize (uint32_t maxSize)
{
  m_maxSize = maxSize;
}

uint32_t
WranMacQueue::GetMaxSize (void) const
{
  return m_maxSize;
}

bool
WranMacQueue::Enqueue (Ptr<Packet> packet, const MacHeaderType &hdrType,
...
Ptr<Packet>
WranMacQueue::Dequeue (MacHeaderType::HeaderType packetType)
{
  if (!IsEmpty ())
    {
...
Ptr<Packet>
WranMacQueue::Dequeue (MacHeaderType::HeaderType packetType, uint32_t availableByte)
{
  if (!IsEmpty ())
    {
...
Ptr<Packet>
WranMacQueue::Peek (GenericMacHeader &hdr) const
{
  if (!IsEmpty ())
    {
...
Ptr<Packet>
WranMacQueue::Peek (GenericMacHeader &hdr, Time &timeStamp) const
{
  if (!IsEmpty ())
    {
      QueueElement element = m_queue.front ();
...
Ptr<Packet>
WranMacQueue::Peek (MacHeaderType::HeaderType packetType) const
{
  if (!IsEmpty ())
    {
      QueueElement element = Front (packetType);
...
Ptr<Packet>
WranMacQueue::Peek (MacHeaderType::HeaderType packetType,
...
uint32_t
WranMacQueue::GetSize (void) const
{
  return m_queue.size ();
}

uint32_t
WranMacQueue::GetNBytes (void) const
{
  return m_bytes;
}

uint32_t WranMacQueue::GetQueueLengthWithMACOverhead (void)
{
...
WranMacQueue::QueueElement
WranMacQueue::Front (MacHeaderType::HeaderType packetType) const
{
...
void
WranMacQueue::Pop (MacHeaderType::HeaderType packetType)
{
  QueueElement element;
...
bool
WranMacQueue::IsEmpty (void) const
{
  return m_queue.empty ();
}

bool
WranMacQueue::IsEmpty (MacHeaderType::HeaderType packetType) const
{
...
const WranMacQueue::PacketQueue &
WranMacQueue::GetPacketQueue (void) const
{
  return m_queue;
}

bool
WranMacQueue::CheckForFragmentation (MacHeaderType::HeaderType packetType)
{
  QueueElement element;
  for (std::deque<QueueElement>::const_iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
...
uint32_t
WranMacQueue::GetFirstPacketHdrSize (MacHeaderType::HeaderType packetType)
{
  QueueElement element;
  for (std::deque<QueueElement>::const_iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
...
uint32_t
WranMacQueue::GetFirstPacketPayloadSize (MacHeaderType::HeaderType packetType)
{
  QueueElement element;
  for (std::deque<QueueElement>::const_iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
...
uint32_t
WranMacQueue::GetFirstPacketRequiredByte (MacHeaderType::HeaderType packetType)
{
...
void
WranMacQueue::SetFragmentation (MacHeaderType::HeaderType packetType)
{
  for (std::deque<QueueElement>::iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
    {
...
void
WranMacQueue::SetFragmentNumber (MacHeaderType::HeaderType packetType)
{
  for (std::deque<QueueElement>::iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
    {
...
void
WranMacQueue::SetFragmentOffset (MacHeaderType::HeaderType packetType, uint32_t offset)
{
  for (std::deque<QueueElement>::iterator iter = m_queue.begin (); iter
       != m_queue.end (); ++iter)
    {
...
void
WranMacQueue::QueueElement::SetFragmentation ()
{
  m_fragmentation = true;
}

void
WranMacQueue::QueueElement::SetFragmentNumber ()
{
  m_fragmentNumber++;
}

void
WranMacQueue::QueueElement::SetFragmentOffset (uint32_t offset)
{
