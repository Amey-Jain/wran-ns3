...
#ifndef WRAN_SS_NET_DEVICE_H
#define WRAN_SS_NET_DEVICE_H
...
class WranSSScheduler;
class WranSSLinkManager;
class WranSSServiceFlowManager;
class WranIpcsClassifier;

/**
 * \ingroup wran
 */
class WranSubscriberStationNetDevice : public WranNetDevice
{
public:
  enum State
  {
  ...
  WranSubscriberStationNetDevice (void);
  WranSubscriberStationNetDevice (Ptr<Node>, Ptr<WranPhy> );
  ~WranSubscriberStationNetDevice (void);

  /**
   * \brief initializes the net device and sets the parameters to the default values
   */
  void InitWranSubscriberStationNetDevice (void);
  /**
   ...
   */
  void SetBasicConnection (Ptr<WranConnection> basicConnection);
  /**
   * \return the basic connection currently in use
   */
  Ptr<WranConnection> GetBasicConnection (void) const;
  /**
   * \param primaryConnection the primary connection to be used
   */
  void SetPrimaryConnection (Ptr<WranConnection> primaryConnection);
  /**
   * returns the primary connection currently usde
   */
  Ptr<WranConnection> GetPrimaryConnection (void) const;
  /**
   ...
   */
  WranPhy::ModulationType GetModulationType (void) const;
  /**
  ...
  /**
   * \param areWranServiceFlowsAllocated true if the service flows are allocated, false otherwise
   */
  void SetAreWranServiceFlowsAllocated (bool areWranServiceFlowsAllocated);
  /**
   * \returns true if the service flows are allocated, false otherwise
   */
  bool GetAreWranServiceFlowsAllocated (void) const;
  /**
   * \return the scheduler installed on the device
   */
  Ptr<WranSSScheduler> GetScheduler (void) const;
  /**
   * \param WranSSScheduler the scheduler to be installed on the device
   */
  void SetScheduler (Ptr<WranSSScheduler> WranSSScheduler);
  /**
   * \returns true if the device has at least one active service flow, false otherwise
   */
  bool HasWranServiceFlows (void) const;
  /**
   ...
   */
  void AddWranServiceFlow (WranServiceFlow *sf);
  /**
   * \brief adds a new service flow
   */
  void AddWranServiceFlow (WranServiceFlow sf);
  ...
  Ptr<WranSSLinkManager> m_linkManager;
  /**
   * \returns a pointer to the CS packet classifier
   */
  Ptr<WranIpcsClassifier> GetWranIpcsClassifier () const;
  /**
   * \brief Sets the packet classifier to be used
   */
  void SetIpcsPacketClassifier (Ptr<WranIpcsClassifier> );
  /**
   * \returns a pointer to the link manger currently used
   */
  Ptr<WranSSLinkManager> GetLinkManager (void) const;
  /**
   * \brief sets the link manager to be used
   */
  void SetLinkManager (Ptr<WranSSLinkManager> );
  /**
   * \returns a pointer to the service flow manager installed on the device
   */
  Ptr<WranSSServiceFlowManager> GetWranServiceFlowManager (void) const;
  /**
...
private:
  ...
  Ptr<WranConnection> m_basicConnection;
  Ptr<WranConnection> m_primaryConnection;
  ...
  bool m_areWranServiceFlowsAllocated;

  Ptr<WranSSScheduler> m_scheduler;
  Ptr<WranSSServiceFlowManager> m_serviceFlowManager;
  Ptr<WranIpcsClassifier> m_classifier;

  TracedCallback<Ptr<const Packet>, Mac48Address, Cid*> m_traceSSRx;
  TracedCallback<Ptr<const PacketBurst>, Mac48Address, Cid*, WranPhy::ModulationType> m_traceSSTx;
