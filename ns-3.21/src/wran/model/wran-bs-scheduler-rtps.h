 ...
 */
class WranBSSchedulerRtps : public WranBSScheduler
{
public:
  WranBSSchedulerRtps ();
  WranBSSchedulerRtps (Ptr<WranBaseStationNetDevice> bs);
  ~WranBSSchedulerRtps (void);
   ...
   */
  bool SelectIRandBCConnection (Ptr<WranConnection> &connection);
  /**
   ...
   * to transmit, true otherwise
   */
  bool SelectMenagementConnection (Ptr<WranConnection> &connection);
  /**
   ...
   */
  bool SelectUGSConnection (Ptr<WranConnection> &connection);
  /**
   * \brief Check for rtPS connections that have packets to transmit.
   */
  bool SelectRTPSConnection (Ptr<WranConnection> &connection);
  /**
   * \brief Check for nrtPS connections that have packets to transmit.
   */
  bool SelectNRTPSConnection (Ptr<WranConnection> &connection);
  /**
   * \brief Check for BE connections that have packets to transmit.
   */
  bool SelectBEConnection (Ptr<WranConnection> &connection);
   ...
   */
  void WranBSSchedulerBroadcastConnection (uint32_t &availableSymbols);

  /*
   * \brief schedules the IR connections
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerInitialRangingConnection (uint32_t &availableSymbols);

  /*
   * \brief schedules the basic connections
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerBasicConnection (uint32_t &availableSymbols);

  /*
   * \brief schedules the primary connection
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerPrimaryConnection (uint32_t &availableSymbols);
  /*
   * \brief schedules the UGS connection
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerUGSConnection (uint32_t &availableSymbols);
  /**
   ...
   */
  void WranBSSchedulerRTPSConnection (uint32_t &availableSymbols);

  /*
   * \brief schedules the NRTPS connections
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerNRTPSConnection (uint32_t &availableSymbols);

  /*
   * \brief schedules the BE connection
   * \param availableSymbols the remaining free OFDM symbols in the current subframe
   */
  void WranBSSchedulerBEConnection (uint32_t &availableSymbols);
