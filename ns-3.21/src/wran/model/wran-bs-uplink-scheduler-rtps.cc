...
NS_LOG_COMPONENT_DEFINE ("WranUplinkSchedulerRtps");

namespace ns3 {
NS_OBJECT_ENSURE_REGISTERED (WranUplinkSchedulerRtps);
  

WranUplinkSchedulerRtps::WranUplinkSchedulerRtps ()
{
  SetBs (0);
...
WranUplinkSchedulerRtps::WranUplinkSchedulerRtps (Ptr<WranBaseStationNetDevice> bs)
{
  SetBs (bs);
  SetTimeStampIrInterval (Seconds (0));
  SetNrIrOppsAllocated (0);
...
WranUplinkSchedulerRtps::~WranUplinkSchedulerRtps (void)
{
  SetBs (0);
...
TypeId
WranUplinkSchedulerRtps::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WranUplinkSchedulerRtps").SetParent<WranUplinkScheduler> ();
  return tid;
}

std::list<OfdmUlMapIe>
WranUplinkSchedulerRtps::GetUplinkAllocations (void) const
{
  return m_uplinkAllocations;
}

void
WranUplinkSchedulerRtps::GetChannelDescriptorsToUpdate (bool &updateDcd, bool &updateUcd, bool &sendDcd, bool &sendUcd)
{
...
uint32_t
WranUplinkSchedulerRtps::CalculateAllocationStartTime (void)
{
  return GetBs ()->GetNrDlSymbols () * GetBs ()->GetPhy ()->GetPsPerSymbol () + GetBs ()->GetTtg ();
}

void
WranUplinkSchedulerRtps::AddUplinkAllocation (OfdmUlMapIe &ulMapIe,
...
void
WranUplinkSchedulerRtps::Schedule (void)
{
  m_uplinkAllocations.clear ();
  SetIsIrIntrvlAllocated (false);
  SetIsInvIrIntrvlAllocated (false);
      ...
      for (std::vector<WranSSRecord*>::iterator iter = ssRecords->begin (); iter != ssRecords->end (); ++iter)
        {
          ...
          if (!ssRecord->GetPollForRanging () && ssRecord->GetRangingStatus ()
              != WranNetDevice::RANGING_STATUS_CONTINUE && ssRecord->GetAreWranServiceFlowsAllocated ())
            {
              OfdmUlMapIe ulMapIe;
              cid = ssRecord->GetBasicCid ();
              ulMapIe.SetCid (cid);
...
void
WranUplinkSchedulerRtps::ServiceUnsolicitedGrants (const WranSSRecord *ssRecord,
  ...
  for (std::vector<WranServiceFlow*>::iterator iter = serviceFlows.begin (); iter != serviceFlows.end (); ++iter)
    {
      ...
      if (serviceFlow->GetSchedulingType () == WranServiceFlow::SF_TYPE_NRTPS)
        {
...
void
WranUplinkSchedulerRtps::ServiceBandwidthRequests (const WranSSRecord *ssRecord,
                                               enum WranServiceFlow::SchedulingType schedulingType,
                                               OfdmUlMapIe &ulMapIe,
                                               const WranPhy::ModulationType modulationType,
                                               uint32_t &symbolsToAllocation,
...
bool
WranUplinkSchedulerRtps::ServiceBandwidthRequests (WranServiceFlow *serviceFlow,
                                               enum WranServiceFlow::SchedulingType schedulingType,
                                               OfdmUlMapIe &ulMapIe,
                                               const WranPhy::ModulationType modulationType,
                                               uint32_t &symbolsToAllocation,
      ...
      if (availableSymbols >= allocSizeSymbols)
        {
          ...
          if (schedulingType == WranServiceFlow::SF_TYPE_NRTPS)
            {
...
void
WranUplinkSchedulerRtps::ULSchedulerRTPSConnection (uint32_t &symbolsToAllocation, uint32_t &availableSymbols)
{
  NS_LOG_INFO ("\tUL Scheduler for rtPS flows");
  NS_LOG_INFO ("\t\tavailableSymbols = " << availableSymbols);
  WranServiceFlowRecord *record_[100];
  uint32_t allocSizeSymbols_[100]; // symbolsRequired for each WranSSRecord
  ...
  for (std::vector<WranSSRecord*>::iterator iter = ssRecords->begin (); iter != ssRecords->end (); ++iter)
    {
      ...
      if (!ssRecord->GetPollForRanging () && ssRecord->GetRangingStatus () != WranNetDevice::RANGING_STATUS_CONTINUE
          && ssRecord->GetAreWranServiceFlowsAllocated ())
        {
          ...
          std::vector<WranServiceFlow*> serviceFlows = ssRecord->GetWranServiceFlows (WranServiceFlow::SF_TYPE_RTPS);
...
void
WranUplinkSchedulerRtps::AllocateInitialRangingInterval (uint32_t &symbolsToAllocation, uint32_t &availableSymbols)
{
...
void
WranUplinkSchedulerRtps::SetupWranServiceFlow (WranSSRecord *ssRecord, WranServiceFlow *serviceFlow)
{
...
void
WranUplinkSchedulerRtps::InitOnce ()
{
}

void
WranUplinkSchedulerRtps::ProcessBandwidthRequest (const BandwidthRequestHeader &bwRequestHdr)
{
}

void
WranUplinkSchedulerRtps::OnSetRequestedBandwidth (WranServiceFlowRecord *sfr)
{
