...
#include "ns3/wran-ss-manager.h"
#include "ns3/wran-bs-uplink-scheduler.h"
#include "wran-connection-manager.h"

NS_LOG_COMPONENT_DEFINE ("WranBSLinkManager");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (WranBSLinkManager);

TypeId WranBSLinkManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WranBSLinkManager")
    .SetParent<Object> ();
  return tid;
}

WranBSLinkManager::WranBSLinkManager (Ptr<WranBaseStationNetDevice> bs)
  : m_bs (bs),
    m_signalQuality (10),
...
WranBSLinkManager::~WranBSLinkManager (void)
{
  m_bs = 0;
}

uint8_t
WranBSLinkManager::CalculateRangingOppsToAllocate (void)
{
 ...
 */
void
WranBSLinkManager::ProcessRangingRequest (Cid cid, RngReq rngreq)
{
  NS_ASSERT_MSG (m_bs->GetState () == WranBaseStationNetDevice::BS_STATE_UL_SUB_FRAME,
                 "Base station: Error while processing ranging request: !BS_STATE_UL_SUB_FRAME");

  Time irIntervalBoundary = Seconds (0);

  if (m_bs->GetWranUplinkScheduler ()->GetIsInvIrIntrvlAllocated ())
    {
      if (m_bs->GetWranUplinkScheduler ()->GetIsIrIntrvlAllocated ())
        {
          irIntervalBoundary = m_bs->GetUlSubframeStartTime ()
            + Seconds ((m_bs->GetWranUplinkScheduler ()->GetNrIrOppsAllocated () + 1) * m_bs->GetRangReqOppSize ()
    ...
    {
      NS_ASSERT_MSG (m_bs->GetWranUplinkScheduler ()->GetIsIrIntrvlAllocated (),
                     "Base station: Error while processing ranging request: IR interval not allocated");

      irIntervalBoundary = m_bs->GetUlSubframeStartTime ()
        + Seconds (m_bs->GetWranUplinkScheduler ()->GetNrIrOppsAllocated () * m_bs->GetRangReqOppSize ()
                   * m_bs->GetSymbolDuration ().GetSeconds ());
    }
...
void
WranBSLinkManager::PerformRanging (Cid cid, RngReq rngreq)
{
  RngRsp rngrsp;
  ...
  if (!decodable)
    {
      ...
      rngrsp.SetRangStatus (WranNetDevice::RANGING_STATUS_CONTINUE); // see Figure 64
...
void
WranBSLinkManager::PerformInitialRanging (Cid cid, RngReq *rngreq, RngRsp *rngrsp)
{

  WranSSRecord *ssRecord = 0;
  bool isOldSS = m_bs->GetWranSSManager ()->IsInRecord (rngreq->GetMacAddress ());
  if (isOldSS)
    {
      ssRecord = m_bs->GetWranSSManager ()->GetWranSSRecord (rngreq->GetMacAddress ());
      // if this fails it would mean the RNG-RSP with success status was not received by the SS
    }
  else
    {
      ssRecord = m_bs->GetWranSSManager ()->CreateWranSSRecord (rngreq->GetMacAddress ());
    }
  ...
  if (isOldSS)
    ...
    {
      m_bs->GetWranConnectionManager ()->AllocateManagementConnections (ssRecord, rngrsp);

      WranPhy::ModulationType modulationType;
      uint8_t diuc = m_bs->GetWranBurstProfileManager ()->GetBurstProfileForSS (ssRecord, rngreq, modulationType);
...
void
WranBSLinkManager::PerformInvitedRanging (Cid cid, RngRsp *rngrsp)
{
  WranSSRecord *ssRecord = m_bs->GetWranSSManager ()->GetWranSSRecord (cid);
...
void
WranBSLinkManager::VerifyInvitedRanging (Cid cid, uint8_t uiuc)
{
...
void
WranBSLinkManager::SetParametersToAdjust (RngRsp *rngrsp)
{
...
void
WranBSLinkManager::AbortRanging (Cid cid, RngRsp *rngrsp, WranSSRecord *ssRecord, bool isOldSS)
{
  rngrsp->SetRangStatus (WranNetDevice::RANGING_STATUS_ABORT);
  ScheduleRngRspMessage (cid, rngrsp);

  if (isOldSS)
    {
      ssRecord->SetRangingStatus (WranNetDevice::RANGING_STATUS_ABORT);
    }
...
void
WranBSLinkManager::AcceptRanging (Cid cid, RngRsp *rngrsp, WranSSRecord *ssRecord)
{
  rngrsp->SetRangStatus (WranNetDevice::RANGING_STATUS_SUCCESS);
  ScheduleRngRspMessage (cid, rngrsp);

  /*Shall not be set until the SS receives the RNG-RSP, as it may be lost etc. may be state field
   is also added to WranSSRecord which then set to SS_STATE_REGISTERED once confirmed that SS has received
   this RNG-RSP, but how to determine that, may be as a data packet is received by the SS*/
  ssRecord->SetRangingStatus (WranNetDevice::RANGING_STATUS_SUCCESS);

  ssRecord->DisablePollForRanging ();
}

void
WranBSLinkManager::ContinueRanging (Cid cid, RngRsp *rngrsp, WranSSRecord *ssRecord)
{
  rngrsp->SetRangStatus (WranNetDevice::RANGING_STATUS_CONTINUE);
  ScheduleRngRspMessage (cid, rngrsp);
  ssRecord->SetRangingStatus (WranNetDevice::RANGING_STATUS_CONTINUE);
}

void
WranBSLinkManager::ScheduleRngRspMessage (Cid cid, RngRsp *rngrsp)
{
  if (rngrsp->GetRangStatus () == WranNetDevice::RANGING_STATUS_SUCCESS || rngrsp->GetRangStatus ()
      == WranNetDevice::RANGING_STATUS_CONTINUE)
    {
      SetParametersToAdjust (rngrsp);
    }
  ...
  p->AddHeader (*rngrsp);
  p->AddHeader (WranManagementMessageType (WranManagementMessageType::MESSAGE_TYPE_RNG_RSP));

  m_bs->Enqueue (p, MacHeaderType (), m_bs->GetConnection (cid));
}

void
WranBSLinkManager::DeallocateCids (Cid cid)
{
  // if necessary, delete entire connections or simply set CIDs to 0
}

uint64_t
WranBSLinkManager::SelectDlChannel (void)
{
...
bool
WranBSLinkManager::ChangeDlChannel (void)
{
...
uint32_t
WranBSLinkManager::GetNewDlChannel (void)
{
...
uint8_t
WranBSLinkManager::GetSignalQuality (void)
{
...
bool
WranBSLinkManager::IsRangingAcceptable (void)
{
