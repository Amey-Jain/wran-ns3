...
bool
PUModel::CheckActive(Time timeNow, Time ts, int puId) {

	Time endTime=timeNow+ts;
	int number=m_puData[puId].number_data;
	double active=false;

	for (int i=0; i<number; i++) {

		// Check if there is an overlapping with the current PU activity	
		if( (m_puData[puId].arrival_time[i]>=timeNow && m_puData[puId].departure_time[i]>=endTime && m_puData[puId].arrival_time[i]<=endTime) || (m_puData[puId].arrival_time[i]<=timeNow && m_puData[puId].departure_time[i]>=endTime) ||
    (m_puData[puId].arrival_time[i]<=timeNow && m_puData[puId].departure_time[i]>=timeNow && m_puData[puId].departure_time[i]<=endTime)) {

			active=true;
			m_puData[puId].detected[i]=true;

	 	}

		// If there is on overlapping, then jump out from the cycle
		if (active || m_puData[puId].arrival_time[i]>endTime)
        	              i=number; 
	}
...
double 
PUModel::Distance(double x, double y, int puId) {
	double dx=(x-m_puData[puId].x_loc)*(x-m_puData[puId].x_loc);
	double dy=(y-m_puData[puId].y_loc)*(y-m_puData[puId].y_loc);
...
double 
PUModel::DistanceFromReceiver(double x, double y, int puId) {

	double dx=(x-m_puData[puId].x_loc_receiver)*(x-m_puData[puId].x_loc_receiver);
	double dy=(y-m_puData[puId].y_loc_receiver)*(y-m_puData[puId].y_loc_receiver);
	double dist=std::sqrt((float)(dx+dy));
