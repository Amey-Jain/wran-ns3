...
#include "ns3/wran-cs-parameters.h"
...
public:
  ...
  bool CheckClassifierMatch (Ipv4Address srcAddress,
                             ...
                             uint16_t dstPort,
                             uint8_t proto) const;
  WranServiceFlow (enum Direction direction);
  WranServiceFlow ();
  WranServiceFlow (const WranServiceFlow & sf);
  WranServiceFlow (uint32_t sfid,
               enum Direction direction,
               Ptr<WranConnection> connection);
  ~WranServiceFlow (void);
  ...
  void CopyParametersFrom (WranServiceFlow sf);

  void SetType (enum Type type);
  enum Type GetType (void) const;
  void SetConnection (Ptr<WranConnection> connection);
  // Can return a null connection is this service flow has not
  // been associated yet to a connection.
  Ptr<WranConnection> GetConnection (void) const;

  void SetIsEnabled (bool isEnabled);
  bool GetIsEnabled (void) const;

  void SetRecord (WranServiceFlowRecord *record);
  WranServiceFlowRecord* GetRecord (void) const;

  // wrapper functions
  Ptr<WranMacQueue> GetQueue (void) const;
  enum WranServiceFlow::SchedulingType GetSchedulingType (void) const;
  ...
  enum WranServiceFlow::SchedulingType GetServiceSchedulingType (void) const;
  ...
  enum WranPhy::ModulationType GetModulation (void) const;
  ...
  void SetServiceSchedulingType (enum WranServiceFlow::SchedulingType);
  ...
  void SetModulation (enum WranPhy::ModulationType modulationType);
...
private:
  ...
  enum WranServiceFlow::SchedulingType m_schedulingType;
  ...
  Type m_type;
  Ptr<WranConnection> m_connection;
  bool m_isEnabled;
  bool m_isMulticast;
  enum WranPhy::ModulationType m_modulationType;
  // will be used by the BS
  WranServiceFlowRecord *m_record;
};
