...
#ifndef WRAN_HELPER_H
#define WRAN_HELPER_H
...
#include "ns3/wran-bs-uplink-scheduler-mbqos.h"
#include "ns3/wran-bs-uplink-scheduler-simple.h"
#include "ns3/wran-bs-uplink-scheduler-rtps.h"
#include "ns3/wran-bs-scheduler.h"
#include "ns3/wran-bs-scheduler-simple.h"
...
/**
 * \brief helps to manage and create WranNetDevice objects
 *
 * This class can help to create a large set of similar
 * WranNetDevice objects and to configure their attributes
 * during creation.
 */

class WranHelper : public PcapHelperForDevice //, public AsciiTraceHelperForDevice
{
public:
  /**
   * Net Device Type
  ...
  /**
   * \brief Create a Wran helper in an empty state.
   */
  WranHelper (void);
  ~WranHelper (void);
  /**
   ...
   *  \param netdevice the type of net device for which you want to enable tracing (WranSubscriberStationNetDevice,
   *   WranBaseStationNetDevice or WranNetDevice)
   ...
   */
  Ptr<WranPhy> CreatePhy (PhyType phyType);
   ...
   */
  Ptr<WranUplinkScheduler> CreateWranUplinkScheduler (SchedulerType schedulerType);
   ...
   * according to selected scheduling mechanism.
   */
  Ptr<WranBSScheduler> CreateWranBSScheduler (SchedulerType schedulerType);
   ...
   * ns3::WranSubscriberStationNetDevice or ns3::WranBaseStationNetDevice
   ...
   */
  void SetPropagationLossModel (SimpleOfdmWranChannel::PropModel propagationModel);
   ...
   */
  Ptr<WranPhy> CreatePhyWithoutChannel (PhyType phyType);
   ...
   */
  Ptr<WranPhy> CreatePhy (PhyType phyType, char * SNRTraceFilePath, bool activateLoss);
  /**
   ...
   * depending on the type parameter) is attached to the shared input channel.
   */
  Ptr<WranNetDevice> Install (Ptr<Node> node,
                               NetDeviceType deviceType,
                               PhyType phyType,
                               Ptr<WranChannel> channel,
                               SchedulerType schedulerType);
...
//  SpectrumManager* GetSpectrumManager (void);
   ...
   *
   */
  WranServiceFlow CreateWranServiceFlow (WranServiceFlow::Direction direction,
                                 WranServiceFlow::SchedulingType schedulinType,
                                 WranIpcsClassifierRecord classifier);

  /**
   * Helper to enable all WranNetDevice log components with one statement
   */
  ...
  *          WranNetDevice should be modified to use a fixed stream
  * \param stream first stream index to use
